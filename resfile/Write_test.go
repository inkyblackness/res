...
func from(data []byte) *Reader {
	reader, _ := ReaderFrom(bytes.NewReader(data))
	return reader
}

func TestWriteReturnsErrorForNilTarget(t *testing.T) {
	err := Write(nil, from(emptyResourceFile()))
	assert.Equal(t, errTargetNil, err)
}

func TestWriteReturnsErrorForNilSource(t *testing.T) {
	err := Write(serial.NewByteStore(), nil)
	assert.Equal(t, errSourceNil, err)
}

func TestWriteFromEmptySource(t *testing.T) {
	emptyFileData := emptyResourceFile()
	store := serial.NewByteStore()
	err := Write(store, from(emptyFileData))
